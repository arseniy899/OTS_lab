...
function RRCFilter = SetParamsRRCFilter(inRRCFilter, ParamsNumber, ...
	...
	% Нужно ли выполнять модуляцию и демодуляцию
		if ~isfield(RRCFilter, 'isTransparent')
			RRCFilter.isTransparent = true;
		else
			% Проверка корректности введённых значений
		end
		
		if ~isfield(RRCFilter, 'rolloff')
			RRCFilter.rolloff  = 0.25;
		else
			if (RRCFilter.rolloff < 0 || RRCFilter.rolloff > 1 )
				if strcmp(LogLanguage, 'Russian')
